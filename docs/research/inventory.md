...
- Date: 2026-08-14
- Branch: `feat/creator-longread-redesign` at `bf154d9` (`feat: add fluid scroll-scrubbed opener parallax`) plus this note.
- Tests: `npm test` pass (14/14)
- Verifier: `npm run verify` output: `manifest ok (excerpt files present; vendor clone not present)`
- Build: `npm run build` pass (`/` and `/_not-found` static)
- Stills: inspected `public/stills/{drop,found,scored,hide,missing,run}.jpg`. No Imagine watermarks, no logos, no readable UI chrome. Drop dossier has only illegible form texture.
- Browser: Chrome DevTools MCP has no Chrome at `/opt/google/chrome/chrome`. Used Playwright Chromium against local `npx next start` on `:3000`. Desktop 1280×800: six `.opener-img` stills scrub with the wheel (`y` −0.06 → 74.3 on a 600px wheel), reverse on scroll-up (back to −0.06), no `pin-spacer`, frames stay `position: relative`. Checklist thumbs match chapters (`hide`/`missing`/`drop`/`found`/`run`/`scored`). Scrim text vs paper `#09090B`: title `#FAFAFA` 19.06:1, kicker `#A1A1AA` 7.76:1. Narrow 390×844: `scrollWidth` 390, TOC `position: static`; long excerpts scroll inside `pre` only. `prefers-reduced-motion: reduce`: stills stay `transform: none` after wheel; `html` `scroll-behavior: auto`.
- Deploy: push `feat/creator-longread-redesign` only (no `vercel --prod`, no `vendor/` upload).
